...
def nn_sample():
    inputs = np.zeros(9)
    inputs[0:3] = np.random.uniform(-5,5,size=3) + np.array([0,-9,0])
    ## random final postion
    inputs[3:6] = np.random.uniform(-2,2,size=3) + np.array([0,6,0])
    ## random initial yaw angle
    inputs[6] = np.random.uniform(-pi/6,pi/6)
    ## random width of the gate
    inputs[7] = np.clip(np.random.normal(0.9,0.2),0.8,1.5)
