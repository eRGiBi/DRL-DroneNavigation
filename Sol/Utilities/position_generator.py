...
class PositionGenerator():
    ...
    def generate_random_point_around_line(self, from_point, to_point):
        ...
        perpendicular_vector = np.cross(direction_vector, random_vector)
        perpendicular_vector = perpendicular_vector / np.linalg.norm(perpendicular_vector)

        # Generate a random offset within the specified range
        offset = random.uniform(-self.max_distance, self.max_distance)

        # Apply the offset in the direction perpendicular to the line
        x_point += offset * perpendicular_vector[0]
        y_point += offset * perpendicular_vector[1]
        z_point += offset * perpendicular_vector[2]
