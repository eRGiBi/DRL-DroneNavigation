class PBDroneEnv(
    ...
    def is_out_of_cylinder_bounds(self, drone_position, circle_center=(0, 0, 1), extension_length=0.2):
        ...
        if self.circle:
            ...
            # Extend the line
            extended_line_point1 = base1 - extension_length * line_unit_vector
            extended_line_point2 = base2 + extension_length * line_unit_vector

            # Project the point onto the extended line segment
            point_vector = point - extended_line_point1
            projection_length = np.dot(point_vector, line_unit_vector)

            # Ensure the projection length is within the bounds of the extended line segment
            projection_length = np.clip(projection_length, 0,
                                        np.linalg.norm(extended_line_point2 - extended_line_point1))

            closest_point_on_line = extended_line_point1 + projection_length * line_unit_vector

            # Calculate the distance from the point to the closest point on the line
            distance_from_line = np.linalg.norm(point - closest_point_on_line)

            print(f"Point: {point}, Closest point on line: {closest_point_on_line}")
            print(f"Distance from line: {distance_from_line}, Threshold: {self._threshold}")
